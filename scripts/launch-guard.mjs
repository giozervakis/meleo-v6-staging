...
if(process.env.RELEASE_TAG!=='v6.2.0') fail.push('RELEASE_TAG must be v6.2.0')
let go
...
let manifest
try{manifest=JSON.parse(fs.readFileSync('reports/release-manifest-v6.2.0.json','utf8'))}catch{fail.push('Run npm run release:manifest before launch')}
if(manifest){
  if(manifest.version!=='6.2.0') fail.push('Release manifest version mismatch')
...
const report={version:'6.2.0',checkedAt:new Date().toISOString(),passed:fail.length===0,failures:fail,warnings:warn}
...
for(const x of fail) console.error('  β—',x)

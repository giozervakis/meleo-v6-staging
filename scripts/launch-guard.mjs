...
if(process.env.RELEASE_TAG!=='v6.2.1') fail.push('RELEASE_TAG must be v6.2.1')
let go
...
let manifest
try{manifest=JSON.parse(fs.readFileSync('reports/release-manifest-v6.2.1.json','utf8'))}catch{fail.push('Run npm run release:manifest before launch')}
if(manifest){
  if(manifest.version!=='6.2.1') fail.push('Release manifest version mismatch')
...
const report={version:'6.2.1',checkedAt:new Date().toISOString(),passed:fail.length===0,failures:fail,warnings:warn}

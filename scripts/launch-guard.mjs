...
if(process.env.RELEASE_TAG!=='v6.1.2') fail.push('RELEASE_TAG must be v6.1.2')
let go
...
let manifest
try{manifest=JSON.parse(fs.readFileSync('reports/release-manifest-v6.1.2.json','utf8'))}catch{fail.push('Run npm run release:manifest before launch')}
if(manifest){
  if(manifest.version!=='6.1.2') fail.push('Release manifest version mismatch')
...
const report={version:'6.1.2',checkedAt:new Date().toISOString(),passed:fail.length===0,failures:fail,warnings:warn}

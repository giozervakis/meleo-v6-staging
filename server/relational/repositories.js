...
export const Bookings={
  ...
  async create(data){await sql(`INSERT INTO bookings(id,patient_id,professional_id,service,visit_date,visit_time,address,notes_encrypted,repeat_rule,status,base_price,patient_contact_consent_at) VALUES($1,$2,$3,$4,$5,$6,$7,$8,$9,'pending',$10,now())`,[data.id,data.patientId,data.professionalId,data.service,data.date,data.time,data.address||'',encryptSensitive(data.notes||''),data.repeat||'Μία φορά',data.price||0]);return this.byId(data.id)},

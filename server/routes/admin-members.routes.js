...
export function registerAdminMembersRoutes({
  ...
  /*
   * TEMPORARY STAGING-ONLY canonical patient repair.
   *
   * Repairs ONLY:
   *   u_patient / patient@meleo.gr
   *
   * Expected accidental professional:
   *   pro_e4c1197f-d8a1-4a91-b69f-5b7a3d65c6c4
   *
   * Remove this endpoint immediately after verification.
   */
  app.post(
    '/api/admin/staging/repair-canonical-patient',
    async(req,res)=>{
      if(
        String(process.env.NODE_ENV||'')
          .trim()
      ...
      if(
        ...
        'REPAIR_CANONICAL_PATIENT'
      ){
        return res.status(400).json({
          error:'Explicit staging repair confirmation required'
        })
      }

      const expectedUserId='u_patient'
      const expectedEmail='patient@meleo.gr'
      ...
      if(!before){
        return res.status(409).json({
          error:'Canonical patient missing; repair refused'
        })
      }

      if(before.email!==expectedEmail){
        return res.status(409).json({
          error:'Canonical patient email mismatch; repair refused'
        })
      }
      ...
      if(
        before.professionalId &&
        before.professionalId!==expectedProfessionalId
      ){
        return res.status(409).json({
          error:'Unexpected professional profile; repair refused',
      ...
      const result=await tx(async client=>{
        ...
        if(locked.rowCount!==1){
          throw new Error(
            'CANONICAL_PATIENT_REPAIR_USER_MISSING'
          )
        }
        ...
        if(row.email!==expectedEmail){
          throw new Error(
            'CANONICAL_PATIENT_REPAIR_IDENTITY_CHANGED'
          )
        }

        if(
          row.professional_id &&
          row.professional_id!==expectedProfessionalId
        ){
          throw new Error(
            'CANONICAL_PATIENT_REPAIR_PROFESSIONAL_CHANGED'
          )
        }
        ...
        if(professionalIds.length){
          ...
          `,[expectedUserId,professionalIds])
        ...
        if(professionalIds.length){
          ...
          if(deletedProfessional.rowCount!==1){
            throw new Error(
              'CANONICAL_PATIENT_REPAIR_PRO_DELETE_FAILED'
            )
          }
        }
        ...
        `,[expectedUserId,expectedEmail])

        if(updated.rowCount!==1){
          throw new Error(
            'CANONICAL_PATIENT_REPAIR_UPDATE_FAILED'
          )
        }

        /*
         * Kill any old patient session that may still carry
         * authorization state from the accidental role.
         */
        await client.query(`
        ...
        return {
          user:updated.rows[0],
          removedProfessionalIds:professionalIds
        }
      })
      ...
      if(
        !verify ||
        verify.email!==expectedEmail ||
        verify.role!=='patient' ||
        verify.professionalId
      ){
        return res.status(500).json({
          error:'Canonical patient post-repair verification failed'
        })
      }
      ...
      return res.json({
        ...
        repaired:true,
        user:verify,
        removedProfessionalIds:
          result.removedProfessionalIds
      })
    }
  )
  /*

...
export default function App(){
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      {view==='become-pro'&&<BecomeProfessional onLogged={logged} user={user} token={token} onRefresh={()=>refreshMe()} setView={setView} setToast={setToast} cfg={cfg}/>}
...
function PatientDashboard({user,token,openPro,cfg,setView,setToast}:any){
 ...
 async function refresh(){const scope=user?.role==='professional'?'&scope=requested':'';const d=await api('/bookings?limit=50'+scope,{},token);setBookings(Array.isArray(d)?d:(d.items||[]))}useEffect(()=>{refresh();const f=()=>refresh();window.addEventListener('meleo:live',f);return()=>window.removeEventListener('meleo:live',f)},[])

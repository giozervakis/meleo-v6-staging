...
function statusLabel(s:string,t:any){
 const labels:any={
  ...
  clarification:t('adminStatus.booking.clarification'),
  quoted:t('adminStatus.booking.quoted'),
  accepted:t('adminStatus.booking.accepted'),
  completed:t('adminStatus.booking.completed'),
  cancelled:t('adminStatus.booking.cancelled')
 }
 return labels[s]||s
}
function professionalLifecycleLabel(s:string,t:any){
 const labels:any={
  approved:t('adminStatus.professionalLifecycle.approved'),
  pending_verification:t('adminStatus.professionalLifecycle.pendingVerification'),
  verification_rejected:t('adminStatus.professionalLifecycle.verificationRejected'),
  awaiting_subscription:t('adminStatus.professionalLifecycle.awaitingSubscription'),
  profile_incomplete:t('adminStatus.professionalLifecycle.profileIncomplete'),
  verification_required:t('adminStatus.professionalLifecycle.verificationRequired'),
  deletion_pending:t('adminStatus.professionalLifecycle.deletionPending')
 }
 return labels[s]||'—'
}
...
const platformOperational=
 ...
 {tab==='verification'&&<div className="panel"><div className="panel-heading"><h3>Verification Operations</h3><span>{t('adminVerification.subtitle')}</span></div>{vers.length?vers.map(v=><div className="admin-row" key={v.id}><div><b>{v.name}</b><span>{v.specialty||t('adminVerification.professionalFallback')} · {v.subscriptionPlan?String(v.subscriptionPlan).toUpperCase():'—'} · {v.city||'—'}</span><span>{v.email||'—'} · {v.phone||'—'}</span><span>{t('adminVerification.license')}: {v.licenseNumber||'—'} · {t('adminVerification.documents')}: {v.documentCount||0}</span>{v.documents?.length>0&&<span className="verification-doc-links">{v.documents.map((d:any)=><a key={d.id} href={`/api/admin/verification-documents/${d.id}`}>{t('adminVerification.download')}: {d.name}</a>)}</span>}<small>{v.createdAt?.slice(0,16).replace('T',' ')}</small></div><span className={'status '+v.status}>{v.status}</span><div className="admin-actions">{v.status!=='approved'&&<button className="positive-lite" onClick={()=>decide(v.id,'approved')}>{t('adminVerification.approve')}</button>}{v.status!=='rejected'&&<button className="danger-lite" onClick={()=>decide(v.id,'rejected')}>{t('adminVerification.reject')}</button>}</div></div>):<Empty title={t('adminVerification.emptyTitle')} text={t('adminVerification.emptyText')}/>}</div>}
 {tab==='support'&&<AdminSupport token={token} setToast={setToast}/>} {tab==='smart'&&<AdminSmartRequests token={token} setToast={setToast}/>} {tab==='audit'&&<div className="panel admin-table-panel"><div className="table-toolbar"><div><h3>Audit Log</h3><span>{t('adminAudit.subtitle')}</span></div><button className="btn ghost small" onClick={refresh}>{t('adminAudit.refresh')}</button></div><div className="responsive-table"><table><thead><tr><th>{t('adminAudit.time')}</th><th>Actor</th><th>Action</th><th>Metadata</th></tr></thead><tbody>{auditRows.map((x:any)=><tr key={x.id}><td>{new Date(x.at).toLocaleString(i18n.resolvedLanguage==='en'?'en-GB':'el-GR')}</td><td><b>{x.actorName}</b><small>{x.actorEmail}</small></td><td><code>{x.action}</code></td><td><small className="audit-meta">{JSON.stringify(x.meta||{})}</small></td></tr>)}</tbody></table></div></div>}
 </div></section>
}

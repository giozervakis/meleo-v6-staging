...
function statusLabel(s:string,t:any){ return ({pending:t('professionalCommon.status.pending'),clarification:t('professionalCommon.status.clarification'),quoted:t('professionalCommon.status.quoted'),accepted:t('professionalCommon.status.accepted'),completed:t('professionalCommon.status.completed'),cancelled:t('professionalCommon.status.cancelled')} as any)[s]||s }
function repeatLabel(r:string,t:any){return ({once:t('professionalCommon.repeat.once'),daily7:t('professionalCommon.repeat.daily7'),twice7:t('professionalCommon.repeat.twice7')} as any)[r]||r}
function money(v:number){return `${Number(v||0).toFixed(2).replace('.',',')}€`}
const FALLBACK_CONFIG={plans:[{id:'basic',name:'BASIC',price:9.99,currency:'EUR',interval:'month',recommended:false,features:['professionalCommon.plan.basicProfile','professionalCommon.plan.basicRequests']},{id:'premium',name:'PREMIUM',price:14.99,currency:'EUR',interval:'month',recommended:true,features:['professionalCommon.plan.premiumAllBasic','professionalCommon.plan.premiumRanking']}]} as any
async function fileToBase64(file:File){return await new Promise<string>((resolve,reject)=>{const r=new FileReader();r.onload=()=>resolve(String(r.result||'').split(',')[1]||'');r.onerror=()=>reject(new Error('FILE_READ_ERROR'));r.readAsDataURL(file)})}
...
function VerifyEmailBanner({user,token,cfg,setToast}:any){
 ...
 async function resend(){setBusy(true);try{const r=await api('/auth/resend-verification',{method:'POST'},token);setToast(r.message||t('professionalCommon.verifyEmail.sent'))}catch(e:any){setToast(e.message)}finally{setBusy(false)}}
 return <div className="verify-email-banner"><div><b>{t('professionalCommon.verifyEmail.title')}</b><span>{t('professionalCommon.verifyEmail.body',{email:user.email})}</span></div><button onClick={resend} disabled={busy}>{busy?t('professionalCommon.verifyEmail.sending'):t('professionalCommon.verifyEmail.resend')}</button></div>
}
function CalendarActions({booking}:any){
  ...
  const title = `MELEO · ${booking.service || t('professionalCommon.calendar.visit')}`
  ...
  return (
    <div className="calendar-actions">
      ...
      <span>
        {t('professionalCommon.calendar.add')}
      </span>
...
function Conversation({messages}:any){const {t,i18n}=useTranslation();if(!messages?.length)return null;return <div className="conversation"><div className="conversation-title">{t('professionalCommon.conversation.title')}</div>{messages.map((m:any)=><div key={m.id} className={'conversation-msg '+m.fromRole}><div><b>{m.fromName}</b><small>{new Date(m.createdAt).toLocaleString(i18n.resolvedLanguage==='en'?'en-GB':'el-GR')}</small></div><p>{m.text}</p></div>)}</div>}
...
function ProfessionalOnboarding({user,professional,token,onRefresh,setToast,cfg}:any){
 ...
 const [verificationEmailBusy,setVerificationEmailBusy]=useState(false)
 ...
 async function uploadVerificationFile(file:File){setError('');if(!emailVerified){setError(t('professionalOnboarding.step4.emailRequired'));return}setUploadBusy(true);try{if(file.size>5*1024*1024)throw new Error(t('professionalOnboarding.errors.fileTooLarge'));const dataBase64=await fileToBase64(file);const d=await api('/professional/verification-document',{method:'POST',body:JSON.stringify({name:file.name,data:dataBase64})},token);setDocs((x:any[])=>[...x,d]);setToast(t('professionalOnboarding.toast.documentUploaded'))}catch(e:any){setError(e.message)}finally{setUploadBusy(false)}}
 async function resendVerificationEmail(){
   setError('')
   setVerificationEmailBusy(true)

   try{
     await api('/auth/verify-email/resend',{method:'POST'},token)
     setVerificationEmailSent(true)
     setToast(t('professionalOnboarding.step4.emailSent'))
   }catch(e:any){
     setError(e.message)
   }finally{
     setVerificationEmailBusy(false)
   }
 }

 async function submitVerification(){setError('');if(!emailVerified){setError(t('professionalOnboarding.step4.emailRequired'));return}if(!vr.licenseNumber.trim()){setError(t('professionalOnboarding.errors.licenseRequired'));return}setBusy(true);try{await api('/professional/verification',{method:'POST',body:JSON.stringify(vr)},token);await onRefresh();setToast(t('professionalOnboarding.toast.verificationSubmitted'))}catch(e:any){setError(e.message)}finally{setBusy(false)}}
 ...
 {step===4&&<div className="onboarding-card"><div className="onboarding-heading"><span>{t('professionalOnboarding.step4.kicker')}</span><h1>{t('professionalOnboarding.step4.title')}</h1><p>{t('professionalOnboarding.step4.intro')}</p></div>{!emailVerified&&<div className="verify-email-banner onboarding-email-gate"><div><b>{t('professionalOnboarding.step4.emailTitle')}</b><span>{t('professionalOnboarding.step4.emailBody',{email:user?.email||''})}</span><small>{t('professionalOnboarding.step4.emailSpamHint')}</small></div><button type="button" onClick={resendVerificationEmail} disabled={verificationEmailBusy||verificationEmailSent}>{verificationEmailBusy?t('professionalOnboarding.step4.emailSending'):verificationEmailSent?t('professionalOnboarding.step4.emailSentButton'):t('professionalOnboarding.step4.emailResend')}</button></div>}<div className="verification-layout"><div className="checkout-form"><label>{t('professionalOnboarding.step4.license')}<input value={vr.licenseNumber} onChange={e=>setVr({...vr,licenseNumber:e.target.value})}/></label><label>{t('professionalOnboarding.step4.documents')}<input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" disabled={uploadBusy||!emailVerified} onChange={e=>{const f=e.target.files?.[0];if(f)uploadVerificationFile(f)}}/><small className="field-hint">{emailVerified?t('professionalOnboarding.step4.fileHint'):t('professionalOnboarding.step4.fileLocked')}</small></label>{docs.length>0&&<div className="uploaded-docs">{docs.map((d:any)=><span key={d.id}>✓ {d.name}</span>)}</div>}<label>{t('professionalOnboarding.step4.notes')}<textarea value={vr.notes} onChange={e=>setVr({...vr,notes:e.target.value})} placeholder={t('professionalOnboarding.step4.notesPlaceholder')}/></label>{error&&<div className="error">{error}</div>}<button className="btn btn-gold wide" disabled={busy||!emailVerified} onClick={submitVerification}>{busy?t('professionalOnboarding.step4.submitting'):t('professionalOnboarding.step4.submit')}</button></div><aside className="checkout-summary"><span>{t('professionalOnboarding.step4.afterSubmit')}</span><h3>{t('professionalOnboarding.step4.pendingTitle')}</h3><p>{t('professionalOnboarding.step4.pendingBody')}</p></aside></div></div>}
...
}}><div className="request-icon">⌂</div><div><b>{b.patientName}</b><span>{b.service}</span><small>{b.date} · {b.time} · {b.address}</small></div><div className="request-price">{b.agreedPrice?`${b.agreedPrice}€`:b.proposedPrice?`${b.proposedPrice}€`:b.price?t('professionalBooking.price.from',{price:b.price}):t('professionalBooking.price.contact')}</div><span className={'status '+b.status}>{statusLabel(b.status,t)}</span>
<button
